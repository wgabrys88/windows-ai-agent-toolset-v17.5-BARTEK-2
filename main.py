...
def mouse_click(x: int, y: int, conv: CoordConverter) -> None:
    ...
    log(f"mouse_click screen=({x},{y}) win32=({ax},{ay})")
    move = INPUT(type=INPUT_MOUSE)
    move.mi = MOUSEINPUT(ax, ay, 0, int(MouseEvent.MOVE | MouseEvent.ABSOLUTE), 0, 0)
    
    down = INPUT(type=INPUT_MOUSE)
    down.mi = MOUSEINPUT(0, 0, 0, int(MouseEvent.LEFT_DOWN), 0, 0)
    
    up = INPUT(type=INPUT_MOUSE)
    up.mi = MOUSEINPUT(0, 0, 0, int(MouseEvent.LEFT_UP), 0, 0)
...
def scroll(dy: float) -> None:
    ...
    log(f"scroll dy={dy} ticks={ticks} direction={direction}")
    inputs = []
...
def _encode_png_rgb(bgra: bytes, width: int, height: int) -> bytes:
    ...
    raw = bytearray(stride_dst * height)
    
    for y in range(height):
        raw[y * stride_dst] = 0
        row_start = y * stride_src
        dst_start = y * stride_dst + 1
        
        for x in range(width):
            src_idx = row_start + x * 4
            dst_idx = dst_start + x * 3
            raw[dst_idx] = bgra[src_idx + 2]
            raw[dst_idx + 1] = bgra[src_idx + 1]
            raw[dst_idx + 2] = bgra[src_idx]
...
def _frame_change_ratio(a: bytes, b: bytes, step: int = 16) -> float:
    if len(a) != len(b) or not a:
        return 1.0
    changed = sum(1 for i in range(0, len(a) - 3, step) if a[i:i+3] != b[i:i+3])
    total = (len(a) - 3) // step + 1
    return changed / total if total else 1.0

def wait_for_screen_settle(conv: CoordConverter) -> None:
    if not SETTLE_ENABLED:
        return
    log("wait_for_screen_settle start")
        ...
        if prev is not None:
            ...
            if ratio <= SETTLE_CHANGE_RATIO_THRESHOLD:
                ...
                if stable >= SETTLE_REQUIRED_STABLE:
                    log(f"wait_for_screen_settle stable={stable}")
                    return
            else:
                stable = 0
        prev = sm
        time.sleep(SETTLE_CHECK_INTERVAL_S)
    log("wait_for_screen_settle timeout")
...
class OverlayManager:
    ...
    def render(self) -> None:
        ...
        for line in all_wrapped:
            rect = w.RECT(HUD_MARGIN, y, HUD_MARGIN + max_width, y + line_height)
...
def test_mode() -> None:
    ...
    test_dir = DUMP_FOLDER / f"test_{timestamp}"
